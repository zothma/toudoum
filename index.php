...
<body>
    ...
    <main  class="main__recherche">
        ...
        <div class = "recommendations">   
            <?php foreach($donnee_nv_movies as $id => $film) {generer_carte($id, $film, popularite($id));}  ?>
        </div>
        <h2>Séries en vogue</h2>
        <div class="recommendations">
            <?php foreach($donnee_nv_series as $id => $film) {generer_carte($id, $film, popularite($id));} ?>
        </div>
        <h2>Films les plus populaires</h2>
        <div class="recommendations">
            <?php foreach($donnee_popu_movies as $id => $film) {generer_carte($id, $film, popularite($id));} ?>
        </div>
    </main>
